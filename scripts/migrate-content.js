...
// Manually defining the tree from content.ts to avoid TS compilation issues
...
const outputDir = path.join(__dirname, '../src/content/nodes');

if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
}

function traverseAndWrite(node, parentId = null) {
    const fileName = `${node.id}.mdoc`;
    ...
    const frontmatter = [
        ...
        node.experimentUrl ? `experimentUrl: ${node.experimentUrl}` : '',
        `gallery: ${JSON.stringify(node.gallery || [])}`,
        '---',
        ''
    ...
    // If file exists, maybe skip? No, overwrite to ensure fresh migration.
    fs.writeFileSync(filePath, frontmatter + content);
    console.log(`Wrote ${fileName}`);

    if (node.children) {
        node.children.forEach(child => traverseAndWrite(child, node.id));
    }
}

// Start migration
traverseAndWrite(contentTree);
